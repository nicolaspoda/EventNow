...
describe('PollsController', () => {
  ...
  it('getEventPolls delegates to service', async () => {
    ...
    expect(mockPollsService.getEventPolls).toHaveBeenCalledWith('user-1', 'event-1');
  });
  ...
  it('createPoll delegates to service', async () => {
    ...
    expect(mockPollsService.createPoll).toHaveBeenCalledWith('user-1', 'event-1', dto);
  });
  ...
  it('vote delegates to service', async () => {
    ...
    expect(mockPollsService.vote).toHaveBeenCalledWith('user-1', 'event-1', 'poll-1', dto);
  });

  it('changeVote delegates to service', async () => {
    const dto = { optionIds: ['option-2'] };
    mockPollsService.changeVote.mockResolvedValue({});
    await controller.changeVote('event-1', 'poll-1', dto as any, mockUser as any);
    expect(mockPollsService.changeVote).toHaveBeenCalledWith('user-1', 'event-1', 'poll-1', dto);
  });
  ...
  it('closePoll delegates to service', async () => {
    ...
    expect(mockPollsService.closePoll).toHaveBeenCalledWith('user-1', 'event-1', 'poll-1');
  });
  ...
  it('deletePoll delegates to service', async () => {
    ...
    expect(mockPollsService.deletePoll).toHaveBeenCalledWith('user-1', 'event-1', 'poll-1');
  });
});
