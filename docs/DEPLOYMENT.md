# Manuel de déploiement — EventNow
...
- **Stripe** (clés test en dev, clés live en prod) — paiement des billets
- **Cloudinary** — upload des images d'événements
- **Google OAuth** (Google Cloud Console) — connexion via Google
- **SMTP** — Gmail avec un mot de passe d'application, ou Mailtrap en dev pour ne pas envoyer de vrais mails

## Déploiement en local

Le fichier `docker-compose.yml` définit quatre services : `postgres`, `redis`, `backend`, `frontend`. Chacun a son `Dockerfile` avec un stage `dev` et un stage `prod` — en local c'est le stage `dev` qui est utilisé, avec le code monté en volume pour le hot-reload.
...
Si le fichier s'appelle autrement, la variable `EVENTNOW_ENV_FILE` permet de le pointer ailleurs (`export EVENTNOW_ENV_FILE=/chemin/vers/.env`) — c'est ce que fait le déploiement automatisé, voir plus bas.
